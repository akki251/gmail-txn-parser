...
try {
  execSync('adb shell content delete --uri content://sms');
} catch (e) {}

console.log(`Sending all ${corpus.length} SMS samples with JSON string escaping to Android emulator...`);

let sent = 0;
for (const item of corpus) {
  let sender = 'BankSMS';
  if (/SBI/i.test(item.sms)) sender = 'SBIBNK';
  else if (/HDFC/i.test(item.sms)) sender = 'HDFCBK';
  else if (/ICICI/i.test(item.sms)) sender = 'ICICIB';
  else if (/Axis/i.test(item.sms)) sender = 'AXISBK';
  else if (/LazyPay/i.test(item.sms)) sender = 'LazyPay';
  else if (/Simpl/i.test(item.sms)) sender = 'Simpl';
  else if (/Amazon/i.test(item.sms)) sender = 'AMZPAY';
  else if (/Paytm/i.test(item.sms)) sender = 'PAYTM';
  else if (/PhonePe/i.test(item.sms)) sender = 'PHONEPE';
  else if (/Google Pay/i.test(item.sms)) sender = 'GPAY';

  const safeJsonText = JSON.stringify(item.sms);
  try {
    execSync(`adb emu sms send ${sender} ${safeJsonText}`);
    sent++;
  } catch (err) {
    console.error(`Failed to send SMS #${item.id}:`, err.message);
  }
}

console.log(`Successfully delivered ${sent} / ${corpus.length} complete SMS alerts into Android emulator!`);
