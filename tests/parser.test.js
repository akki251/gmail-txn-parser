/**
 * Transaction Parser Regression Test Suite — Web Branch (gmail-sms-shortcut)
 *
 * Layers:
 *   Layer 1 — nonTransactional gate
 *   Layer 2 — deterministic parser (smsParsers + bankParsers)
 *   Layer 3 — mocked LLM fallback contract (no OpenRouter calls in CI)
 *   Layer 4 — full end-to-end pipeline
 *
 * Usage:
 *   node tests/parser.test.js
 *   VERBOSE=1 node tests/parser.test.js
 */
...
// The real OpenRouter call is NEVER made in CI.
async function mockLlmFallback(rawText) {
  // Multiline HDFC "Sent" format
  if (/^Sent\s+Rs\.\d/m.test(rawText)) {
    const amtM = rawText.match(/Sent\s+Rs\.([\d,]+\.?\d*)/i);
    const toM   = rawText.match(/To\s+(.+)/i);
    return {
  ...
  // LazyPay purchase
  if (/lazypay.*purchase.*successful/i.test(rawText)) {
    const amtM = rawText.match(/Rs\.([\d,]+\.?\d*)/i);
    const merM = rawText.match(/at\s+([A-Z]+)/i);
    return {
      amount: amtM ? parseFloat(amtM[1].replace(/,/g, '')) : 0,
      merchant: merM ? merM[1] : 'LazyPay',
      type: 'debit',
      bank: null,
      instrument: null,
      last4: null,
  ...
  // Future/scheduled/offer/promotional language → notATransaction
  if (/\b(will be|scheduled|offer|eligible|approved|initiated|outstanding bill|due|pending|avail|pre-approved)\b/i.test(rawText)) {
    return { notATransaction: true };
  }

  // Bill payment received → notATransaction
  if (/\b(received payment|payment.*received|bill payment.*received)\b/i.test(rawText)) {
    return { notATransaction: true };
  }

  // Generic: amount + completion verb
  const amtM = rawText.match(/(?:Rs\.?|INR|₹)\s*([\d,]+\.?\d*)/i);
  const hasVerb = /\b(debited|credited|spent|paid|withdrawn|transferred|disbursed|charged|refunded)\b/i.test(rawText);
  if (amtM && hasVerb) {
    return {
      amount: parseFloat(amtM[1].replace(/,/g, '')),
      merchant: null,
      type: /credited|refund|reversal|disbursed/i.test(rawText) ? 'credit' : 'debit',
      bank: null,
      instrument: null,
      last4: null,
      currency: 'INR',
...
async function runLayer1() {
  ...
  // Gate must NEVER block a real transaction
  for (const tc of corpus.filter(c => c.expected.classification === 'TRANSACTION')) {
    const blocked = isNonTransactional(tc.sms);
    if (blocked) {
  ...
  // Gate CAN catch non-transactions (but doesn't have to — deeper layers handle the rest)
  for (const tc of corpus.filter(c => c.expected.classification === 'NON_TRANSACTION')) {
    const blocked = isNonTransactional(tc.sms);
    if (blocked) {
...
async function runLayer2() {
  section('Layer 2 — Deterministic Parser (smsParsers + bankParsers)');
  ...
  for (const id of knownDeterministic) {
    ...
    if (isNonTransactional(tc.sms)) {
      tc.expected.classification === 'NON_TRANSACTION' ? pass(id) : fail(id, 'Gate blocked a TRANSACTION case');
      continue;
    }

    const result = parseTransactionSms({ sender: 'TEST-BANK', text: tc.sms });

    if (tc.expected.classification === 'NON_TRANSACTION') {
      if (!result || result.needsLLMFallback || result.notATransaction) {
        pass(id);
      } else {
        fail(id, `Parser produced transaction for NON_TRANSACTION case. Got: ${JSON.stringify(result)}`);
      }
    } else {
      if (!result || result.needsLLMFallback) {
...
async function runLayer3() {
  section('Layer 3 — LLM Fallback Contract (mock OpenRouter)');
  ...
  const llmCases = [
    ...
    {
      id: 'llm-future-emi-non-txn',
      sms: 'Your HDFC Bank account will be debited by Rs.24999 for EMI.',
      expected: { classification: 'NON_TRANSACTION' },
    },
    {
      id: 'llm-axis-loan-offer-non-txn',
      sms: 'Avail your Axis Bank Personal Loans offer of Rs.500000 starting @15.25% p.a.',
      expected: { classification: 'NON_TRANSACTION' },
    },
    {
      id: 'llm-onecard-repayment-non-txn',
      sms: 'Hola! that was sweet. We have received payment against your OneCard for Rs. 4,137.69 on 17 Aug 2026.',
      expected: { classification: 'NON_TRANSACTION' },
    },
  ];
  ...
  for (const tc of llmCases) {
    ...
    if (tc.expected.classification === 'TRANSACTION') {
      if (tc.expected.amount !== undefined && Math.abs(aiResult.amount - tc.expected.amount) > 0.01) {
        fail(tc.id, `amount: expected ${tc.expected.amount}, got ${aiResult.amount}`); continue;
      }
      if (tc.expected.type && aiResult.type !== tc.expected.type) {
        fail(tc.id, `type: expected "${tc.expected.type}", got "${aiResult.type}"`); continue;
      }
    }
    pass(tc.id);
  }
