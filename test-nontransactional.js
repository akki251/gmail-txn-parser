...
const fixtures = [
  {
    ...
    expectFiltered: true,
    expectedClassification: Classification.NON_TRANSACTION,
  },
  {
  ...
  },
  {
    ...
    expectedClassification: Classification.TRANSACTION,
  },
  {
  ...
  // --- Two-stage Classification: Mandate / Ambiguous sample ---
  {
    label: 'Mandate revocation — should be classified as AMBIGUOUS for Stage 1 LLM verification',
    text: 'Your UPI mandate has been successfully revoked towards Google Asia Pacific Pte.Ltd for INR 5600.00 - Axis Bank',
    expectFiltered: false,
    expectedClassification: Classification.AMBIGUOUS,
  },
];
...
for (const fx of fixtures) {
  ...
  const classification = classifyMessage(fx.text);
  console.log(`--- ${fx.label} ---`);
  console.log(`  filtered: ${result} (classification: ${classification})`);
  const filterMatch = result === fx.expectFiltered;
  const classMatch = classification === fx.expectedClassification;
  if (filterMatch && classMatch) {
    pass++;
  } else {
    if (!filterMatch) console.log(`  ✗ expected filtered=${fx.expectFiltered}, got ${result}`);
    if (!classMatch) console.log(`  ✗ expected classification=${fx.expectedClassification}, got ${classification}`);
  }
}
